...
export const ToneZigzag = React.memo(function ToneZigzag({
  ...
  return (
    ...
    >
      <div className={companion ? "theory-hue-plots" : undefined}>
        {companion}
        <div className="theory-hue-zigzag">
          <svg
                    ...
                    onClick={
                      onSelectEdge
                        ? (event) => {
                            ...
                            const x = ((event.clientX - bounds.left) / bounds.width) * VB_W;
                            onSelectEdge(Math.max(0, Math.min(5, Math.floor(((x - ML) / PW) * 6))));
                          }
                        : undefined
                    }
                  />
            ...
            {CANONICAL_HUE_EDGES.map((edge, index) => {
              ...
              return (
                <tr key={`row-${index}`} data-edge-row={index} data-hue-selected={selectedEdge === index}>
                  ...
                  <td style={TABLE_CELL_STYLE}>
                    {levelLabel(fromLevel)} {relation} {levelLabel(toLevel)}
                  </td>
                </tr>
              );
            })}
